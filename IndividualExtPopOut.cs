...
// Base code by Vidar Ersson & Maverick I. N.
// Individual Extension by Maverick I. N.

public class GameLogic {    // Handles the logic of the game
    
    public bool is_human_turn = true, move_made = false, game_on = false, winner = false;
    ...
    public void StartGameButton(object obj, EventArgs eventArgs)
    {
        ...
        if(gui.popOutMode){ gui.ApplyCss(gui.label, $"#Connect4{{ color: red;}}"); gui.label.Text = "Connect 4 - Pop-out"; }
        else{ gui.ApplyCss(gui.label, $"#Connect4{{ color: green;}}"); gui.label.Text = "Connect 4 - Place"; }
        
    }
    public void RandomMoveButton(object obj, EventArgs eventArgs)
    {
        gui.ApplyCss(gui.label, $"#Connect4{{ color: white;}}"); gui.label.Text = "Connect 4";
        game_on = false;
...
public class GUI : Window{  // Handles drawing the buttons, WinScreen and OnKeyPressEvent
    ...
    public Label label = new Label("Connect 4");
    public Button play_game_button, fill_grid_button;
    public bool popOutMode = false;
    ...
    public GUI(int rows, int cols, int cell_size, GameLogic gameLogic) : base("Connect 4"){
        ...
        label = new Label("Connect 4");
        label.Name = "Connect4";
        ApplyCss(label, $"#Connect4{{ background-image:none; color: white; padding: 10px; font-size: 30px;}}");
    ...
    protected override bool OnKeyPressEvent(Gdk.EventKey ev)
    {
        ...
        else if (ev.Key >= Gdk.Key.Key_1 && ev.Key <= Gdk.Key.Key_9 && GameLogic.game_on)
        {
            ...
            if(popOutMode && !GameLogic.move_made){
                GameLogic.move_made = GameLogic.PopOut(column);
            }
            else{
...
public class Program{
    public static void Main()   // Implementing the Pop-out variant of the individual extension
    {
        Application.Init();
